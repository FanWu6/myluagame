...
function RichLabel:playFadeInAnim(wordPerSec)
    ...
    print("aadad---"..table.maxn(spriteArray))
    if spriteArray then
...
--                    self:removeNodeEventListenersByEvent(cc.NODE_ENTER_FRAME_EVENT)
            ...
            self:unscheduleUpdate()
            self:scheduleUpdateWithPriorityLua(updatePosition,0) 
        end
    end
end
...
function RichLabel:init_(params)
	...
	--装文字和图片精灵
	local containLayer = cc.Layer:create()
...
function RichLabel:parseString_(str)
	...
	-- 普通格式label显示
	if table.maxn(clumpheadTab) == 0 then
		local ptab = {}
		ptab.text = str
