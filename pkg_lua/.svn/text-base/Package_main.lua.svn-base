...
require "src/pkg_lua/pkg_handbook"
...
local function getDataByType(type)
    ...
    for i,v in ipairs(bagItemData) do
        --5为碎片，背包不展示
        if type > 0 and v._itemData.iType ~= 5 and v._itemData.iType == type and v._itemData.iNum>v._itemData.iheroNum then
            table.insert(result,v)
        elseif type == 0 and v._itemData.iType ~= 5 and v._itemData.iNum>v._itemData.iheroNum then
            table.insert(result,v)
        end
    end
...
function paclage_scrollInit_change(type,_page,_Name,_Num)   --背包内物品改变
    ...
    if type == "delete" then
        ...
        if currData[1] ~= nil then
            ...
            DetailInit(_pdetail,xx,2,_currentpage) --package_detal  层，数据，界面打开类型
        end
...
--    end
    ...
    local function _ptouchEvent(sender,eventType)
        ...
        elseif eventType == ccui.TouchEventType.ended then --end操作
            ...
            DetailInit(_pdetail,currItemData,2,_currentpage) --package_detal  层，数据，界面打开类型
            testData[1] = _pdetail
...
--继承BASELAYER后初始化
local LogInPKGLayer = class("LogInPKGLayer",BaseLayer)

function LogInPKGLayer.extend(target)
    local t = tolua.getpeer(target)
    if not t then
        t = {}
        tolua.setpeer(target, t)
    end
    setmetatable(t, LogInPKGLayer)
    return target
end

function LogInPKGLayer:onEnter()
    cclog("LogInPKGLayer:onEnter")
    --菜单初始化
...
function LogInPKGLayer:onExit()
    cclog("LogInPKGLayer:onExit")
...
function LogInPKGLayer:init()
    cclog("LogInPKGLayer:init")
end

function LogInPKGLayer.create()
    local layer = LogInPKGLayer.extend(cc.Layer:create())
    cclog("LogInPKGLayer:create")
...
--继承BASELAYER后初始化 end
function packageLogInMain()
--runFucn
    local function runFunc()
         testData ={nil}
        local _uilayer = LogInPKGLayer.create()
        pkgscene:addChild(_uilayer)
    ...
    --run
    pkgscene = cc.Scene:create()
    if cc.Director:getInstance():getRunningScene() then
        if _uisceneStack[table.maxn(_uisceneStack)] ~= "pkgscene" then
                pushUIScene(pkgscene,"pkgscene")
                runFunc()
        end
      
    else
        cc.Director:getInstance():runWithScene(pkgscene)
    end
end
